...
static int st7586_probe(struct spi_device *spi)
{
	...
	/* TODO: visual and line_length need to be set when setting mode */
	info->fix.visual = FB_VISUAL_STATIC_PSEUDOCOLOR;
	info->fix.line_length = (data->xres * 2 + 7) / 8;
	/* align to a multiple of 12 for st7586_update_display()  */
	...
	ret = fb_alloc_cmap(&info->cmap, info->var.bits_per_pixel, 0);
	if (ret < 0)
		goto err0;
